...
function transposeMatrix(matrixType) {
    let matrix;
    if (matrixType === 'first') {
        matrix = JSON.parse(localStorage.getItem('firstMatrix'));
    } else if (matrixType === 'second') {
        matrix = JSON.parse(localStorage.getItem('secondMatrix'));
    }

    if (matrix) {
        let transposedMatrix = matrix[0].map((col, i) => matrix.map(row => row[i]));
        localStorage.setItem('resultMatrix', JSON.stringify(transposedMatrix));
        document.getElementById("resultMatrix").innerHTML = `Transposed Matrix:<br>${writingMatrixInTable(transposedMatrix)}`;
        document.getElementById("resultMatrix").style.display = "block";
    } else {
        alert("Matrix not found. Please create the matrix first.");
    }
}
